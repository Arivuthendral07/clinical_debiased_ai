...
3. **Blinded RAG Retrieval (ChromaDB)** — Queries a local ChromaDB vector store using strictly anonymized symptoms, applying a mathematical distance threshold so the vector search isn't skewed by patient demographics or polluted by low-relevance guidelines.
...
3. Pull the required models into your local Ollama instance:
```bash
   ollama pull llama3
